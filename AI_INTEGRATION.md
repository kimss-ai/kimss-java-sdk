...
- Existing OpenAI or Anthropic app: keep that client. Set the Kimss base URL, the Gateway key, the vaulted model alias, and `X-Kimss-Agent-Id`. Java services use the vendor Java client or JDK `HttpClient`, not `com.kimss:kimss-java`.
- No LLM client yet, and the user wants a new Python agent: `pip install kimss-forge` and `gateway="kimss"`. Repo: https://github.com/kimss-ai/kimss-forge
